...
void ForEachEvent::SerializeTo(SerializerElement& element) const {
  ...
  if (!orderBy.GetPlainString().empty()) {
    element.AddChild("orderBy").SetStringValue(orderBy.GetPlainString());
  }
  if (!order.empty()) {
    element.AddChild("order").SetStringValue(order);
  }
  if (!limit.GetPlainString().empty()) {
    element.AddChild("limit").SetStringValue(limit.GetPlainString());
  }
}
...
void ForEachEvent::UnserializeFrom(gd::Project& project,
  ...
  orderBy = gd::Expression(
      element.HasChild("orderBy")
          ? element.GetChild("orderBy").GetStringValue()
          : "");
  order = element.HasChild("order")
          ? element.GetChild("order").GetStringValue()
          : "";
  limit = gd::Expression(
      element.HasChild("limit")
          ? element.GetChild("limit").GetStringValue()
          : "");
}
