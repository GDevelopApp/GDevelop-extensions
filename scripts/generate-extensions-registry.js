...
const { validateExtension, validateNoDuplicates } = require('./lib/ExtensionValidator')
 ...
 */
const writeJSONFile = (path, object) => fs.writeFile(path, JSON.stringify(object, null, 2))
...
;(async () => {
  try {
    ...
    const allExtensionWithFileInfos = [...reviewedExtensionWithFileInfos, ...communityExtensionWithFileInfos]
    ...
    await Promise.all(
      allExtensionWithFileInfos.map(async (extensionWithFileInfo) => {
        ...
        if (extensionsWithoutValidation.has(name)) {
          ...
          if (errors.length !== 0) {
            shell.echo(`\n❌ ${errors.length} Error${errors.length > 1 ? 's' : ''} found in extension '${name}':\n`)
            errors.forEach((error) => {
              totalErrors++
              if (error.fix)
                if (args['fix']) {
        ...
        if (args['fix']) await writeJSONFile(path.join(extensionsBasePath, tier, `${name}.json`), extension)
        ...
        const extensionHeader = {
          ...
          description: Array.isArray(extension.description) ? extension.description.join('\n') : extension.description,
        ...
        await writeJSONFile(path.join(distExtensionsPath, `${name}.json`), extension)

        await writeJSONFile(path.join(distExtensionsPath, `${name}-header.json`), extensionHeader)
      })
    )

    if (totalErrors) {
      shell.echo(
        `\n\n❌ ${totalErrors} Error${totalErrors > 1 ? 's' : ''} found in extensions - please fix ${
    ...
    const views = JSON.parse(await fs.readFile(path.join(extensionsBasePath, 'views.json'), 'utf8'))
    ...
    await writeJSONFile(path.join(distDatabasesPath, 'extensions-database.json'), registry)

    console.log(`✅ Headers and registry files successfully updated`)
  } catch (error) {
    console.error(`⚠️ Error while generating headers and registry files:`, error)
    shell.exit(1)
  }
})()
