...
CommonInstructionsExtension::CommonInstructionsExtension() {
  ...
  GetAllEvents()["BuiltinCommonInstructions::ForEach"].SetCodeGenerator(
      [](gd::BaseEvent &event_, gd::EventsCodeGenerator &codeGenerator,
        ...
        // When orderBy is set, we need a pre-pass to evaluate the orderBy
        // expression for each object, then sort, and optionally limit.
        if (hasOrderBy) {
          // We need a separate context for the pre-pass to evaluate orderBy
          // for each object individually.
          gd::EventsCodeGenerationContext orderByContext;
          orderByContext.InheritsFrom(parentContext);
          orderByContext.ForbidReuse();
          for (unsigned int i = 0; i < realObjects.size(); ++i)
            orderByContext.EmptyObjectsListNeeded(realObjects[i]);

          gd::String orderByObjectDeclaration =
              codeGenerator.GenerateObjectsDeclarationCode(orderByContext) + "\n";

          gd::String orderByExprCode =
              gd::ExpressionCodeGenerator::GenerateExpressionCode(
                  codeGenerator, orderByContext, "number",
                  event.GetOrderBy());

          gd::String prePassIndexVar =
              codeGenerator.GetCodeNamespaceAccessor() + "forEachPrePassIndex" +
              gd::String::From(context.GetContextDepth());
          codeGenerator.AddGlobalDeclaration(prePassIndexVar + " = 0;\n");

          gd::String sourceList;
          if (realObjects.size() == 1) {
            sourceList = codeGenerator.GetObjectListName(realObjects[0], parentContext);
          } else {
            sourceList = forEachObjectsList;
          }

          // Build an array of {obj, val} pairs
          outputCode += forEachSortedList + ".length = 0;\n";
          outputCode += "for (" + prePassIndexVar + " = 0; " + prePassIndexVar + " < " +
                        sourceList + ".length; ++" + prePassIndexVar + ") {\n";

          // Pick one object for expression evaluation
          outputCode += orderByObjectDeclaration;
          ...
          if (realObjects.size() == 1) {
            ...
            outputCode += temporary + " = " + sourceList + "[" + prePassIndexVar + "];\n";
            outputCode += codeGenerator.GetObjectListName(realObjects[0], orderByContext) +
                          ".push(" + temporary + ");\n";
          } else {
            for (unsigned int i = 0; i < realObjects.size(); ++i) {
              gd::String count;
              ...
              outputCode += "if (" + prePassIndexVar + " < " + count + ") {\n";
              outputCode += "    " +
                            codeGenerator.GetObjectListName(realObjects[i], orderByContext) +
                            ".push(" + forEachObjectsList + "[" + prePassIndexVar + "]);\n";
              outputCode += "}\n";
            }
          }

          outputCode += forEachSortedList + ".push({obj: " + sourceList +
                        "[" + prePassIndexVar + "], val: " + orderByExprCode + "});\n";
          outputCode += "}\n"; // End of pre-pass loop

          // Sort the array
          bool isDesc = event.GetOrder() == "desc";
          if (isDesc) {
            outputCode += forEachSortedList + ".sort(function(a, b) { return b.val - a.val; });\n";
          } else {
            outputCode += forEachSortedList + ".sort(function(a, b) { return a.val - b.val; });\n";
          }

          // Apply limit
          if (!event.GetLimit().empty()) {
            gd::String limitExprCode =
                gd::ExpressionCodeGenerator::GenerateExpressionCode(
            ...
            outputCode += forEachLimitVar + " = " + limitExprCode + ";\n";
            outputCode += "if (" + forEachLimitVar + " > 0 && " + forEachSortedList +
                          ".length > " + forEachLimitVar + ") " + forEachSortedList +
                          ".length = " + forEachLimitVar + ";\n";
          }
        }
        ...
        // Pick one object
        if (hasOrderBy) {
          // When sorting, pick from the sorted list
          gd::String sortedObj = forEachSortedList + "[" + forEachIndexVar + "].obj";

          if (realObjects.size() == 1) {
            outputCode += codeGenerator.GetObjectListName(realObjects[0], context) +
                          ".push(" + sortedObj + ");\n";
          } else {
            // Need to figure out which list the object belongs to
            for (unsigned int i = 0; i < realObjects.size(); ++i) {
              if (i != 0) outputCode += "else ";
              outputCode += "if (" + sortedObj + ".getName() === \"" + realObjects[i] + "\") {\n";
              outputCode += "    " +
